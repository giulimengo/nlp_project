...
def main():
    ...
    log_path = project_root / "logs" / "simplewiki_bm25_no_llm_all_competitions.csv"
    ...
    print(f"Loading BM25 logs from {log_path.name}...")
    ...
    print("\n--- BM25 Strategy Summary ---")
    print(summary.to_string(index=False))
    
    # Plot Accuracy
    plt.figure(figsize=(10, 6))
    sns.barplot(data=summary, x='competition_name', y='accuracy')
    plt.title('BM25 Accuracy per Competition')
    plt.xticks(rotation=45, ha='right')
    plt.ylim(0, 1)
    plt.tight_layout()
    acc_plot_path = figures_dir / "bm25_accuracy.png"
    ...
    plt.title('BM25 Response Latency per Competition')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    lat_plot_path = figures_dir / "bm25_latency.png"
